...
class _AddCategoryState extends State<AddCategory> {
  ...
  if (selectedImageFromGallery != null && selectedImageFromGallery is File) {
    ...
    final savedImage = await selectedImageFromGallery.copy('${appDir.path}/$fileName');
  ...
  Future<void> _addCategory() async {
    ...
    try {
      ...
      if (categoryId > 0) {
        ...
        if (mounted) {
        Navigator.pop(context); // Ajoutez cette ligne
      }
  ...
  try {
    ...
    final newImagePath = _categoryToEdit!.imagePath != _selectedImage!.path 
        ? _selectedImage!.path 
        : _categoryToEdit!.imagePath;
 ...
 Future<void> _editSubCategory(SubCategory subCategory) async {
  ...
  if (result && mounted) {
    try {
      // Créer une copie mise à jour de la sous-catégorie
      ...
      final updateResult = await _sqldb.updateSubCategory(updatedSubCategory);
      
      if (updateResult > 0) {
        // Mettre à jour la liste locale
        setState(() {
          _subCategories = _subCategories.map((sc) {
            return sc.id == subCategory.id ? updatedSubCategory : sc;
          }).toList();
        });
        _showMessageSuccess("Sous-catégorie mise à jour avec succès");
      } else {
        _showMessage("Échec de la mise à jour de la sous-catégorie");
      }
    } catch (e) {
      _showMessage("Erreur lors de la mise à jour: $e");
    }
