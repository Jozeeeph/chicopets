...
class Addorder {
                                ...
                                if (selectedPaymentMethod == "Mixte")
                                  Column(
                                    children: [
                                      ...
                                      SizedBox(height: 10),
                                      Text(
                                        "Total saisi: ${(cashAmount + cardAmount + checkAmount + ticketRestaurantAmount + (voucherAmount)).toStringAsFixed(2)} DT",
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold),
                                      ),
                                    ],
  ...
  static double _calculateMaxGlobalDiscountPercentage(
    ...
    double minIndividualMaxDiscount =
        products.map((p) => p.remiseMax).reduce((a, b) => a < b ? a : b);
    ...
    for (int i = 0; i < products.length; i++) {
      ...
      double prixAchat = product.prixHT;
      double profitAvantRemise = (product.marge / 100) * prixAchat;
      double remiseMaxPourcentage = product.remiseMax;

      double maxDiscountValuePerUnit =
          (remiseMaxPourcentage / 100) * profitAvantRemise;

      totalProfit += maxDiscountValuePerUnit * quantity;
      totalPrice += prixTTC * quantity;
    }

    double maxDiscountFromProfit =
        totalPrice > 0 ? (totalProfit / totalPrice) * 100 : 0.0;
    return min(maxDiscountFromProfit, minIndividualMaxDiscount)
        .clamp(0.0, 100.0);
  }
  ...
  static double _calculateMaxGlobalDiscountValue(
    ...
    // Trouver la remise valeur max la plus restrictive parmi tous les produits
    double minRemiseValeurMax = products.first.remiseValeurMax;

    for (final product in products) {
      if (product.remiseValeurMax < minRemiseValeurMax) {
        minRemiseValeurMax = product.remiseValeurMax;
      }
    }

    return minRemiseValeurMax;
  }
